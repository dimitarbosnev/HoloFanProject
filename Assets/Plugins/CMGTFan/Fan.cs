...
namespace CMGT
{
    public static class Fan
    {
        ...
        public static int TextureSize = 672;
        ...
        public static void ProjectOnDisplay(in byte[] array)
        {
            byte[] bytes = TypeConversion.GetBytes(array.Length, true);

            byte[] source = new byte[4 + array.Length];
            source[0] = Protocol.ProHeader;
            source[1] = bytes[0];
            source[2] = bytes[1];
            source[3] = bytes[2];
            Array.Copy(array, 0, source, 4, array.Length);
